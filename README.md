...
支持卡组编辑撤回，以及单人练习、AI、本机双开、双方兼容客户端的局域网两人对战撤回。人工对战支持 Single 和 Match；Match 只可撤回当前未结束小局，换备及下一局沿用原版流程。朋友局逐次确认；AI 仍为 Single，Tag 和观战暂未开放。当前为开发测试版，第二台电脑和人工视觉验收待完成。
...
当前源码还包含原版联机流程调整：普通响应和计时复用原处理方法，联机仅比较必要协议/恢复格式和逻辑卡数据，客机不扫描效果脚本；另附随包卸载工具。以上 alpha.1 下载包尚不包含这些改动，旧撤回握手不能与新的 v2 包混用。卸载工具确认后先备份再移除可验证的 mod 文件，默认保留原游戏资源、配置和日志。
