...
$testDir=Join-Path $out ('room-action-mainloop-'+$id);Assert-PackagePath $testDir $out
...
$exe=Join-Path $testDir 'room_action_mainloop_tests.exe'
Push-Location $build
try{& $compiler '-std=c++17' '-fno-rtti' '-static' '-g' @defs @includes '-I../gframe' '-I../../tests' (Join-Path $root 'tests/room_action_mainloop_tests.cpp') @objects @libs '-o' $exe *> (Join-Path $testDir 'build.log');if($LASTEXITCODE){throw "MainLoop compile failed: $testDir/build.log"}}finally{Pop-Location}
...
$stageName='room-action-mainloop-runtime-'+$id
