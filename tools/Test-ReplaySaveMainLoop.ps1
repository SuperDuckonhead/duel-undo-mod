param([string]$RuntimeRoot='F:/MyCardLibrary/ygopro',[ValidateSet('Debug','Release')][string]$Configuration='Release',[ValidateSet('save','cancel','edit-save','auto-save')][string]$Scenario='edit-save',[string]$BotDirectory='')
...
$testDir=Join-Path $out ('replay-save-mainloop-'+$id);Assert-PackagePath $testDir $out
...
$exe=Join-Path $testDir 'replay_save_mainloop_tests.exe'
Push-Location $build
try{& $compiler '-std=c++17' '-fno-rtti' '-static' '-g' @defs @includes '-I../gframe' '-I../../tests' (Join-Path $root 'tests/replay_save_mainloop_tests.cpp') @objects @libs '-o' $exe *> (Join-Path $testDir 'build.log');if($LASTEXITCODE){throw "MainLoop compile failed: $testDir/build.log"}}finally{Pop-Location}
...
$stageName='replay-save-mainloop-runtime-'+$id
