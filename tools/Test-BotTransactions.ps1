...
$build = Join-Path $repoRoot "out/tests/W2/$Configuration"
$logs = Join-Path $repoRoot "out/w2/$Configuration"
...
try {
    ...
    & $cmake --build $build --target bot_transaction_tests -j2 *> (Join-Path $logs 'build-native.log')
    if($LASTEXITCODE) { throw "Native build failed; see $logs/build-native.log" }
    & $ctest --test-dir $build -R '^bot_transaction_tests$' --output-on-failure *> (Join-Path $logs 'native-tests.log')
    ...
    Write-Host "W2 transaction verification passed ($Configuration); logs: $logs"
} finally {
    Pop-Location
