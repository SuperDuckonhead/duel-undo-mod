...
- Each tool extraction writes a tool-specific incomplete marker before changing its destination and removes it only after its expected probe exists. A later run rejects a marked tool even if that probe was already written.
...
Observed validation: six focused fixture cases pass (verified extraction, checksum mismatch, forbidden destination, offline missing archive, incomplete source directory, interrupted tool extraction with an existing probe). The current baseline's five tool archives and fourteen library archives pass Offline verification. This is not yet an online acquisition into an empty cache or a full clean-checkout rebuild of the finished mod; those checks remain part of release acceptance.
