...
Single 计时双开、自由双开、真实 AI 菜单/私有 bot、默认 RoomClient 分配失败/恢复与 256 字节响应，以及计时先于选择提示的回归均通过；日志为 `out/match-validation/final-*.log`。Match 专用双进程主用例设置无倒计时以精确验证撤回事务保存的时钟值；带 180 秒倒计时的额外完整 Match 和安装包结果另记交付记录。

第二台物理电脑、跨网络连接和两边人工操作仍待用户配合验收。本次自动双进程测试不关闭这项待办。首次 AI 启动等待、此前独立记录的建房问题不在本次改动范围内。
