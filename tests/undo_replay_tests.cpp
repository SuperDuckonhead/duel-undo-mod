...
int main(){try{
 ...
 rejected=false;try{loaded.CreateUndoDriver(changed);}catch(const std::exception&){rejected=true;}CHECK(rejected);CHECK(SamePosition(original->Current().checkpoint,oldRoute.checkpoint));
...
// Legacy uniform replay save/open/response remains the existing codec.
 ...
 std::filesystem::current_path(previous);std::cout<<"actual A-undo-B-finish-save-open-play, 256-byte response, parameter/seed fidelity, memory-only record and legacy playback passed\n";
