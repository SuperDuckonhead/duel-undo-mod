...
- 完整 AI 宿主 10/100/1000 目标规模，各 20 次成功/失败恢复的测量尚未完成。短档正式运行在 20 次成功后发现一次句柄数变化，补充的 20+20 诊断仅一次 168→169 后稳定；三次同进程分支析构后为 155、155、155，子进程均为零，尚不对额外句柄来源作无证归因。正式三规模测量仍待完成，原失败保留。内核层已有独立测量，见 performance.md。
- 干净检出从空缓存 Bootstrap 后的候选构建、精确 ZIP/源码对应、原目录安装并存校验。
